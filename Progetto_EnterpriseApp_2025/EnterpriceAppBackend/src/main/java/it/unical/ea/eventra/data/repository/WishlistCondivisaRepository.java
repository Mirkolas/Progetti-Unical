...
public interface WishlistCondivisaRepository extends JpaRepository<WishlistCondivisa, Long>, JpaSpecificationExecutor<WishlistCondivisa> {
    ...
    @Query("SELECT CASE WHEN COUNT(wc) > 0 THEN true ELSE false END FROM WishlistCondivisa wc WHERE wc.wishlist.id = :wishlistId")
    boolean isWishlistCondivisa(@Param("wishlistId") Long wishlistId);
}