...
import it.unical.ea.eventra.data.entity.*;
import it.unical.ea.eventra.data.repository.StrutturaRepository;
import it.unical.ea.eventra.dto.EventoEditDTO;
...
public class EventoServiceImpl implements EventoService {
    ...
    @Override
    public EventoDTO update(Long id, EventoEditDTO eventoDTO) {
        validateId(id);
        ...
        evento.setDataOraEvento(eventoDTO.getDataOraEvento());
        evento.setDataOraAperturaCancelli(eventoDTO.getDataOraAperturaCancelli());

        Evento updatedEvento= eventoRepository.save(evento);

        return toDto(updatedEvento);
    }
    ...
    public byte[] generaICS(Long eventoId) {
        ...
        String sb = "BEGIN:VCALENDAR\n" +
                "VERSION:2.0\n" +
                "PRODID:-//Eventra//EventraApp//EN\n" +
                "METHOD:PUBLISH\n" +
                "BEGIN:VEVENT\n" +
                "UID:" + UUID.randomUUID() + "\n" +
                "DTSTAMP:" + LocalDateTime.now().format(formatter) + "\n" +
                "DTSTART:" + inizio.format(formatter) + "\n" +
                "DTEND:" + fine.format(formatter) + "\n" +
                "SUMMARY:" + sanitize(evento.getNome()) + "\n" +
                "DESCRIPTION:" + sanitize(evento.getDescrizione()) + "\n" +
                "LOCATION:" + sanitize(evento.getStruttura().getNome()) + "\n" +
                "END:VEVENT\n" +
                "END:VCALENDAR";
    ...
    private EventoDTO toDto(Evento evento) {
        ...
        dto.setStrutturaId(evento.getStruttura() != null ? evento.getStruttura().getId() : null);
    ...
    private Evento toEntity(EventoDTO dto) {
        ...
        if (dto.getStrutturaId() != null) {
            Struttura struttura = strutturaRepository.findById(dto.getStrutturaId())
                    .orElseThrow(() -> new NotFound("Struttura non trovata con id: " + dto.getStrutturaId()));
