...
public class BigliettoController {
    ...
    @Operation(
            summary = "conta biglietti",
            description = "conta quanti biglietti sono stati acquistati per ogni evento cosi da mostrarlo all'organizzatore"
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "conteggio effettuato con successo"),
            @ApiResponse(responseCode = "404", description = "evento non trovato"),
            @ApiResponse(responseCode = "500", description = "Errore interno del server")
    })
    @GetMapping("/count/{id}")
    public ResponseEntity<Long> countBiglietti(@PathVariable Long id){
        bigliettoService.countBigliettiByEvento(id);
        return ResponseEntity.ok(bigliettoService.countBigliettiByEvento(id));
    }
}
