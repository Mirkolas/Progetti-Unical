...
public class UtenteController {
    ...
    @Operation(
            ...
            description = "Crea un nuovo account utente. Restituisce un JWT, refresh token e ID utente in caso di successo."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Registrazione avvenuta con successo"),
            @ApiResponse(responseCode = "400", description = "Email già in uso o dati mancanti/invalidi"),
            @ApiResponse(responseCode = "500", description = "Errore interno del server durante la registrazione")
    })
    ...
    public ApiResponseConfiguration<ResponseAuthentication> register(@Valid @RequestBody UtenteDTO utenteDTO) {
        try {
            ...
            Utente user = utenteRepository.findByEmail(utenteDTO.getEmail())
            ...
            wishlistDTO.setUtenteId(user.getId());
    ...
    @Operation(
            ...
            description = "Aggiorna nome, cognome e numero di telefono dell'utente autenticato tramite JWT."
    )
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "403", description = "Utente non autorizzato a modificare questo profilo"),
            @ApiResponse(responseCode = "500", description = "Errore interno del server")
    })
    @PutMapping("/aggiorna/{userId}")
    public ApiResponseConfiguration<String> aggiornaProfilo(
        ...
        try {
            ...
            String numeroTelefono = body.get("numerotelefono");

            boolean updated = utenteservice.aggiornaProfilo(userId, usernameFromToken, nome, cognome, numeroTelefono);

            if (updated) {
                return new ApiResponseConfiguration<>(true, "Profilo aggiornato con successo", "Profilo aggiornato");
            } else {
                return new ApiResponseConfiguration<>(false, "Utente non autorizzato o non trovato", null);
            }

        } catch (Exception e) {
            return new ApiResponseConfiguration<>(false, "Errore durante l'aggiornamento del profilo: " + e.getMessage(), null);
        }
    }
    ...
    @Operation(
            ...
            description = "Restituisce le informazioni dell'utente dato il suo ID. Se l'utente non esiste, restituisce un messaggio di errore."
    )
    @ApiResponses(value = {
    ...
    public ApiResponseConfiguration<UtenteDTO> getUserData(@PathVariable @Min(1) Long id) {
        try {
            ...
            utenteDTO.setNumerotelefono(utenteDTO.getNumerotelefono());
            utenteDTO.setRole(utente.getRole().name());

            return new ApiResponseConfiguration<>(true, "Dati utente recuperati con successo", utenteDTO);
        } catch (Exception e) {
    ...
    @PutMapping("/cambiaPassword/{userId}")
    public ApiResponseConfiguration<String> cambiaPassword(
            @PathVariable Long userId,
            @RequestBody Map<String, String> body,
            HttpServletRequest request) {
        ...
        try {
            ...
            boolean result = utenteservice.aggiornaPassword(usernameFromToken, userId, oldPassword, newPassword);

            if (result) {
                return new ApiResponseConfiguration<>(true, "Password aggiornata con successo", "Password aggiornata");
            } else {
                return new ApiResponseConfiguration<>(false, "Password attuale errata o utente non autorizzato", null);
            }
    ...
    @Operation(
            summary = "Cerca utente tramite email",
            description = "Restituisce i dati dell'utente associato a una determinata email."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Utente trovato"),
            @ApiResponse(responseCode = "404", description = "Utente non trovato con l'email specificata"),
    ...
    public ResponseEntity<Long> getIdByEmail(@RequestParam String email) {
        return utenteRepository.findByEmail(email)
                .map(utenteDTO -> ResponseEntity.ok(utenteDTO.getId()))
